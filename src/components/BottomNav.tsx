...
export default function BottomNav() {
  ...
  const navItems = [
    ...
    { path: "/profile", label: "Profile", icon: user && user.photoURL ? (
      <Image src={user.photoURL} alt="Profile" width={28} height={28} className="w-6 h-6 sm:w-7 sm:h-7 rounded-full border-2 border-[#fff200] object-cover" />
    ) : "👤" },
  ];

  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 mobile-safe-left mobile-safe-right mobile-safe-bottom">
      ...
      <div className="relative bg-black/90 backdrop-blur-md border-t-4 border-[#fff200] shadow-2xl">
        <div className="flex justify-evenly items-center p-2 sm:p-4">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center gap-1 p-2 sm:p-3 rounded-xl transition-all duration-300 transform hover:scale-110 relative touch-target mobile-tap ${
                  isActive
              ...
              >
                <span className="text-xl sm:text-2xl filter drop-shadow-lg flex items-center justify-center">{item.icon}</span>
